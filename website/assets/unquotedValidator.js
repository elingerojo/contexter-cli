...
function unquotedValidator (value) {
  ...
  var regexIdentifierName = new RegExp('^[' + identifierStart + '][' + identifierStart + identifierPartExclusive + ']*$');
  ...
  function isNumericLiteral(str, num) {
      // Consider: empty string, `2e2`, `010`, ` 010` (leading space), `1.23`, `.23`, `+1`, `-0`, etc.
      return regexNumber.test(str) && !isNaN(num);
  }

  function isIdentifierName(str) {
      return regexIdentifierName.test(str);
  }

  function isES3Reserved(str) {
      return regexES3ReservedWord.test(str);
  }

  function isZeroWidth(str) {
      return regexZeroWidth.test(str);
  }
  ...
  var valueAsUnescapedString = value.replace(regexUnicodeEscape, function($0, $1) {
  ...
  if (isIdentifierName(valueAsUnescapedString)) {
      es3Warning = needsQuotes = isES3Reserved(valueAsUnescapedString) || isZeroWidth(valueAsUnescapedString);
      if (needsQuotes) {
          quotedValue = '\'' + value + '\'';
      }
      needsBrackets = false;
  ...
  return {
      ...
      es3Warning: es3Warning,
      quotedValue: quotedValue || value
  };
